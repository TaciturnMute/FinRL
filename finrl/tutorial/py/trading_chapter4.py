...
def parser_args():
    ...
    data_group.add_argument('--indicators', type=str, default="macd,boll_ub,boll_lb,rsi_30,cci_30,dx_30,close_30_sma,close_60_sma", help='使用的技术指标')
    data_group.add_argument('--if_indicator_norm', action='store_true', help='是否对技术指标执行归一化, 默认为False')
    data_group.add_argument('--if_price_norm', action='store_false', help='是否对价格执行归一化，默认为True')
    data_group.add_argument('--if_num_share_norm', action='store_true', help='是否对持股数，默认为False')
    ...
    env_group.add_argument('--cash_norm_factor', type=float, default=1e-6, help='现金缩放因子，取1表示不缩放')
    env_group.add_argument('--num_share_norm_factor', type=float, default=1, help='持股数缩放因子，取1表示不缩放')
    env_group.add_argument('--risk_indicator_col', type=str, default='vix', help='预警因子')

    policy_group = parser.add_argument_group('策略参数组')
    policy_group.add_argument('--M', type=int, default=1, help='MLP结构激活函数')
    policy_group.add_argument('--N', type=int, default=2, help='MLP结构激活函数')
    ...
    agent_group.add_argument('--batch_size', type=int, default=3, help='小批量大小')
    agent_group.add_argument('--n_updates', type=int, default=3, help='每次小批量抽取的样本的训练次数')
    ...
    agent_group.add_argument('--task', type=str, default='trading', help='投资任务')
    agent_group.add_argument('--q_target_mode', type=str, default='redq', help='集成学习方式')
    agent_group.add_argument('--total_updates_times_maximum', type=int, default=int(90000), help='总训练次数')
    agent_group.add_argument('--mcts_gamma', type=float, default=0.99, help='Backprop阶段更新参数')
    agent_group.add_argument('--expand_length', type=int, default=200, help='Expand阶段交互长度')
    agent_group.add_argument('--children_maximum', type=int, default=3, help='结点最大孩子数')
    agent_group.add_argument('--mcts_C', type=float, default=0.5, help='平衡探索和利用的参数')
    agent_group.add_argument('--random_select_prob', type=float, default=0.5, help='Select阶段，控制继续深入或生成新结点的参数')
...
def get_df(args):
    ...
    return df_train, df_validation, df_test, df_train_validation, args
...
def init_env(args, df_train, df_validation, df_test, df_train_validation):
    ...
    args.state_dim = 1 + action_dim * (2 + len(args.indicators))
    ...
    env_train = StockTradingEnv(
        ...
        num_stock_shares=[0]*args.stock_dim,
        tech_indicator_list=args.indicators,
        cash_norm_factor=args.cash_norm_factor,
        num_share_norm_factor=args.num_share_norm_factor,
        if_price_norm=args.if_price_norm,
        if_indicator_norm=args.if_indicator_norm,
        if_num_share_norm=args.if_num_share_norm,
    ...
    env_validation = StockTradingEnv(
        df=df_validation,
        stock_dim=stock_dim,
        state_dim=args.state_dim,
        hmax=args.hmax,
        ...
        turbulence_threshold=validation_vix_threshold,
    )
    ...
    env_test= StockTradingEnv(
        ...
        risk_indicator_col=args.risk_indicator_col,
        turbulence_threshold=test_vix_threshold,
    )
...
def main():
    ...
    df_train, df_validation, df_test, df_train_validation, args = get_df(args)
    print('data is reday \n')
